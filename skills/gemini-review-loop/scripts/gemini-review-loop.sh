#!/usr/bin/env bash
#
# gemini-review-loop.sh — drive the GitHub Codex (@codex) review loop on a PR.
#
# The mechanical half of the gemini review loop:
...
#       Print {"available": true|false|"unknown", "via": "<source>"} for the Gemini
...
die() { echo "gemini-review-loop.sh: $*" >&2; exit 2; }
...
classify_json() {
  jq --arg cleanre "$CLEAN_REGEX" --arg bannerre "$BANNER_REGEX" --arg defbot "$DEFAULT_BOT" '
    ...
    | (.since // "")                    as $since
    # normalize a login like "gemini-review-connector[bot]" -> base name
    ...
    # non-actionable "💡 Gemini Review" wrapper banner, which carries no actionable content.
...
case "$ACTION" in
  ...
  detect)
    ...
    #    scoped to *other* repos, so for each matching gemini installation we resolve
    ...
    for ep in "user/installations" "orgs/$owner/installations"; do
      ...
      # For each gemini installation, mark repoIncluded: "all" => true; "selected" =>
  ...
  poll)
    ...
    # latest Gemini activity past page 1. Fail loudly on API error rather than silently
